...
def renderizar_fornecedores():
    ...
    # Paginação com setas e número de página
    col_seta_esq, col_info, col_seta_dir = st.columns([1, 10, 1])
    with col_seta_esq:
        if st.button("◀") and pagina > 1:
            st.session_state["pagina_fornecedores"] = pagina - 1
    with col_info:
        st.markdown(f"<div style='text-align: center; font-size: 13px;'>Página {pagina} de {paginas}</div>", unsafe_allow_html=True)
    with col_seta_dir:
